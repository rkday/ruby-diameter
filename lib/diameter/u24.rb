...
# Command-Codes but not representable by {String#pack} or {Array#unpack}.
module UInt24

  # @api private
  #
  # Generates an unsigned integer from two other unsigned integers (one representing
  # the top 8 bits, one representing the bottom 16 bits).
  #
  ...
  # @api private
  #
  # Converts an unsigned integer to two other unsigned integers (one representing
  # the top 8 bits, one representing the bottom 16 bits).
  #
  # @param twentyfourb [Fixnum] The original number (max 2^24-1)
  # @return [[Fixnum, Fixnum]]  Separate integers representing he top 8 and low 16 bits.
